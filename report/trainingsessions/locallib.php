...
class trainingsessions {
     ...
     * @return boolean
     */
    public function fill_structure_from_flexiblesections(&$structure,&$itemcount,  $parentid = null) {
        global $DB, $COURSE;
        static $parents;
...
public function format_time($timevalue, $mode = 'html') {
    if ($timevalue) {
        switch ($mode) {
            ...
            case 'html': // anciennement avec strftime
                $dt = new \DateTime();
                $dt->setTimestamp($timevalue);
                // Format : 2025-06-05 14:45 (Thu)
                return $dt->format('Y-m-d H:i (D)');

            case 'xlst': // Excel - jour fractionnaire
                return 25569 + $timevalue / DAYSECS;

            case 'xlsd': // Excel - heure avec secondes
                $secs = $timevalue % 60;
                $mins = floor($timevalue / 60);
                $hours = floor($mins / 60);
                $mins = $mins % 60;
            ...
            default: // ancien strftime(get_string(...))
                $dt = new \DateTime();
                $dt->setTimestamp($timevalue);
                $format = get_string('strfdatetime', 'report_trainingsessions'); // ex. 'l d M Y, H:i'
                // Remplace si besoin les formats `strftime()` par les équivalents `DateTime::format()`
                // Exemple si format Moodle est `%A %d %B %Y` → 'l d F Y'
               // $format = $this->convert_strftime_to_dateformat($format);
               // return $dt->format($format);
return userdate($timevalue, $format);
        }

    } else {
        // cas où $timevalue est null ou 0
        switch ($mode) {
            case 'html':
                return get_string('unvisited', 'report_trainingsessions');
            case 'htmld':
