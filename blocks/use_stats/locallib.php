...
$sessionid = $log->sessionid ?? 'unknown';
    ...
    if (!isset($aggregate[$log->$dimension])) {
        $aggregate[$log->$dimension] = [];
    }
    if (!isset($aggregate[$log->$dimension][$log->cmid])) {
    ...
    if (!isset($aggregate['realmodule'])) {
        $aggregate['realmodule'] = [];
    }
    if (!isset($aggregate['realmodule'][$log->cmid])) {
    ...
    $aggregate['realmodule'][$log->cmid]->lastaccess = $log->time;
            ...
            if (array_key_exists('coursetotal', $aggregate) && array_key_exists($log->course, $aggregate['coursetotal'])&& is_object($aggregate['coursetotal'][$log->course])) {
                ...
                @$aggregate['coursetotal'][$log->course]->lastaccess = $log->time;
                        ...
                        // Initialiser l'objet avant d'assigner des propriétés
                       if (!isset($aggregate['coursetotal'])) {
                         $aggregate['coursetotal'] = [];
                        }
                        $aggregate['coursetotal'][$log->course] = new stdClass();
                         $aggregate['coursetotal'][$log->course]->elapsed = $lap;
                         $aggregate['coursetotal'][$log->course]->events = 1;
                         $aggregate['coursetotal'][$log->course]->firstaccess = $log->time;
                         $aggregate['coursetotal'][$log->course]->lastaccess = $log->time;
